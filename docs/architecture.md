...
| Resume exactly | Persist per-text: topmost visible block id + fractional offset within it, written on a 500 ms debounce to localStorage. Restore by block id, not pixel offset — survives font-size changes, reflows, app death, phone death. The block-anchor also powers position-keeping across re-renders (audit §3). **Status:** the block-anchor half is built (`anchorKept` across re-renders; `restoreBlock` across a navigation); persistence to localStorage is not, so nothing yet survives app death. |
| Return from a cross-link | A cross-link out of a passage (an instruction naming a prayer) records where the reader stood — text id, block id, and the block's viewport offset — in `js/trail.js`. A labelled bar above the bottom controls, plus a chip at the foot of the text, take them back to that block at that offset. Held in memory, cleared by returning or by any explicit navigation; a prayer chain keeps the deeper origin. No `history` wiring: `display: standalone` means an installed app has no browser back button, so the way back is the app's own. |
